...
export class GooglePlayPurchaseHandler extends PurchaseHandler {
    ...
    */
      ...
      try {
        ...
        const purchaseData: Omit<NonSubscriptionPurchase, "userId"> =
                {
                  ...
                  productId: productData.productId,
                  purchaseDate: firestore.Timestamp.fromMillis(parseInt(
                      response.data.purchaseTimeMillis ?? "0", 10)),
                  status: [
                    "COMPLETED",
                    "CANCELLED",
                    "PENDING",
        ...
        try {
          ...
          console.log("Could not create or update purchase",
              {orderId: response.data.orderId, productId: productData.productId}
          );
        }
        return true;
      } catch (e) {
    ...
    async handleSubscription(
      ...
      try {
        ...
        const purchaseData: Omit<SubscriptionPurchase, "userId"> = {
          ...
          productId: productData.productId,
          purchaseDate: firestore.Timestamp.fromMillis(parseInt(
              response.data.startTimeMillis ?? "0", 10)),
          expiryDate: firestore.Timestamp.fromMillis(parseInt(
              response.data.expiryTimeMillis ?? "0", 10)),
          status: [
    ...
    /** @this GooglePlayPurchaseHandler */
    handleServerEvent = functions.pubsub.topic(GOOGLE_PLAY_PUBSUB_BILLING_TOPIC)
        .onPublish(async (message) => {
          ...
          const notificationType = subscriptionId ? "SUBSCRIPTION" :
          sku ? "NON_SUBSCRIPTION" : null;
          ...
          switch (notificationType) {
            ...
            case "NON_SUBSCRIPTION":
              // eslint-disable-next-line no-invalid-this
              await this.handleNonSubscription(null, productData,
                  purchaseToken);
              break;
          }
        });
