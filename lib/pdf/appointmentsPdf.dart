...
class AppointmentsPdf {
  ...
  List<TableRow> tableChildren(bool fullPage) {
    ...
    for (DocumentSnapshot apt in documents) {
      children.add(TableRow(children: [
        tableField('${apt['rank']} ${apt['name']}, ${apt['firstName']}',
            fullPage ? 3.0 : 2.375),
        tableField(apt['aptTitle'], fullPage ? 2.0 : 1.5),
        tableField(apt['date'], fullPage ? 1.75 : 1.375),
        tableField(apt['start'], fullPage ? 1.125 : 0.875),
        tableField(apt['end'], fullPage ? 1.125 : 0.875)
      ]));
    }
    return children;
  }
